...
<head>
    ...
    <title>Panel de administración de clientes | tumedic</title>
    ...
    $query = "SELECT name FROM admins WHERE email = ?";
...
<body>
    ...
    <section class="bg-gray-50 dark:bg-gray-900 p-3 sm:p-5 antialiased">
        <div class="mx-auto max-w-screen-xl px-4 lg:px-12">
            <div class="flex items-center">
                <h2 class="text-xl font-semibold text-gray-900 dark:text-white sm:text-2xl md:text-2xl lg:text-2xl mb-4 mt-4">
                    Bienvenido<?php echo $name; ?>
                </h2>
                <a href="logout.php" class="mb-4 mt-4 ml-auto flex items-center justify-center text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-4 py-2 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                    Cerrar sesión
                    <?xml version="1.0" ?><svg fill="none" height="24" viewBox="0 0 24 24" width="24" xmlns="http://www.w3.org/2000/svg"><path d="M15 3H7C5.89543 3 5 3.89543 5 5V19C5 20.1046 5.89543 21 7 21H15" stroke="white" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/><path d="M19 12L15 8M19 12L15 16M19 12H9" stroke="white" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"/></svg>
                </a>
            </div>
                    ...
                    <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
                        ...
                        <div class="flex items-center space-x-3 w-full md:w-auto">
                           ...
                           <div id="actionsDropdown" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                <div class="py-1">
                                    <a href="#" id="deleteAllButton" class="block py-2 px-4 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Eliminar todos</a>
                                </div>
                            </div>
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th scope="col" class="px-4 py-4">Correo electrónico</th>
                                <th scope="col" class="px-4 py-4">Contraseña</th>
                                <th scope="col" class="px-4 py-3">CURP</th>
                                <th scope="col" class="px-4 py-3">URL plataforma</th>
                        ...
                        <tbody>
                            ...
                            foreach ($customers as $index => $customer) {
                                $optionsId = "options_" . $index;
                                $buttonId = "options-button-" . $index;
                                echo <<<HTML
                                    <tr class="border-b dark:border-gray-700">
                                        <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{$customer['name']}</th>
                                        <td class="px-4 py-3"><a href="mailto:{$customer['email']}" class="hover:text-primary-600 hover:underline hover:decoration-sky-500">{$customer['email']}</a></td>
                                        <td class="px-4 py-3">{$customer['password']}</td>
                                        <td class="px-4 py-3">{$customer['curp']}</td>
                                        <td class="px-4 py-3"><a class="hover:text-primary-600 hover:underline hover:decoration-sky-500" href="{$customer['platform_url']}" target="_blank">{$customer['platform_url']}</a></td>
                                        <td class="px-4 py-3 flex items-center justify-end">
                                            <button id="' . $buttonId . '" data-dropdown-toggle="' . $optionsId . '" class="inline-flex items-center text-sm font-medium hover:bg-gray-100 dark:hover:bg-gray-700 p-1.5 dark:hover-bg-gray-800 text-center text-gray-500 hover:text-gray-800 rounded-lg focus:outline-none dark:text-gray-400 dark:hover:text-gray-100" type="button">
                                                <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewbox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M6 10a2 2 0 11-4 0 2 2 0 014 0zM12 10a2 2 0 11-4 0 2 2 0 014 0zM16 12a2 2 0 100-4 2 2 0 000 4z" />
                                                </svg>
                                            </button>
                                            <div id="' . $optionsId . '" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                                                <ul class="py-1 text-sm" aria-labelledby="options-button">
                                                    <li>
                                                        <button type="button" onclick="showUpdateModal('{$customer['id']}', '{$customer['name']}', '{$customer['email']}', '{$customer['password']}', '{$customer['curp']}', '{$customer['platform_url']}')" class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200">
                                                    ...
                                                    <li>
                                                        <a href="{$customer['platform_url']}" target="_blank" class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white text-gray-700 dark:text-gray-200">
                                                            <svg class="w-4 h-4 mr-2" xmlns="http://www.w3.org/2000/svg" viewbox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                                    ...
                                                    <li>
                                                        <button type="button" data-modal-target="deleteModal" data-modal-toggle="deleteModal" data-user-id="{$customer['id']}" class="flex w-full items-center py-2 px-4 hover:bg-gray-100 dark:hover:bg-gray-600 text-red-500 dark:hover:text-red-400">
   ...
   <div id="toast-success-massive-delete" class="hidden fixed bottom-0 right-0 transform translate-x-[-50%] mb-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800 w-full max-w-md p-4">
        <div class="flex items-center">
            ...
            <div class="ms-3 text-sm font-normal">Usuarios eliminados correctamente.</div>
        </div>
        <button type="button" class=" absolute top-0 right-0 ms-auto mt-4 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8 dark:text-gray-500 dark:hover:text-white dark:bg-gray-800 dark:hover:bg-gray-700" data-dismiss-target="#toast-success" aria-label="Cerrar">
    ...
    <div id="toast-success-singles-delete" class="hidden fixed bottom-0 right-0 transform translate-x-[-50%] mb-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800 w-full max-w-md p-4">
        <div class="flex items-center">
            <div class="inline-flex items-center justify-center flex-shrink-0 w-8 h-8 text-green-500 bg-green-100 rounded-lg dark:bg-green-800 dark:text-green-200">
                <svg class="w-5 h-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5Zm3.707 8.207-4 4a1 1 0 0 1-1.414 0l-2-2a1 1 0 0 1 1.414-1.414L9 10.586l3.293-3.293a1 1 0 0 1 1.414 1.414Z"/>
            ...
            <div class="ms-3 text-sm font-normal">Usuario eliminado correctamente.</div>
        </div>
        <button type="button" class=" absolute top-0 right-0 ms-auto mt-4 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8 dark:text-gray-500 dark:hover:text-white dark:bg-gray-800 dark:hover:bg-gray-700" data-dismiss-target="#toast-success" aria-label="Cerrar">
            <span class="sr-only">Cerrar</span>
            <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
 ...
 <script>
    document.addEventListener("DOMContentLoaded", function () {
        const deleteModalButtons = document.querySelectorAll('[data-modal-target="deleteModal"]');
    ...
    document.addEventListener('DOMContentLoaded', function () {
        // Cerrar modal al hacer clic en el ícono de cerrar
        document.getElementById('closeUpdateUserModal').addEventListener('click', function () {
    ...
    function showUpdateModal(userId, name, email, password, curp, url) {
        // Rellenar los campos del formulario con los datos del usuario
        document.querySelector('#updateUserModal [name="id"]').value = userId;
        document.querySelector('#updateUserModal [name="name"]').value = name;
        document.querySelector('#updateUserModal [name="email"]').value = email;
        document.querySelector('#updateUserModal [name="password"]').value = password;
        document.querySelector('#updateUserModal [name="curp"]').value = curp;
        document.querySelector('#updateUserModal [name="url"]').value = url;
    ...
    document.getElementById('deleteAllButton').addEventListener('click', function () {
        ...
        .then(message => {
            document.getElementById('toast-success-massive-delete').classList.remove('hidden');
            setTimeout(function() {
                location.reload();
            }, 2300);
        })
